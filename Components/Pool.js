import React, {Component} from 'react';

import {View, StyleSheet, Image, Text, ImageBackground, Button, ScrollView } from 'react-native';
import { Rating, AirbnbRating } from 'react-native-elements';

export default class MainPage extends Component {

    render() {
      const { navigate } = this.props.navigation;
        return(
        <ScrollView>
           <View style={styles.containerTop}>
                 <ImageBackground source={require('../assets/pool.jpg')}
        style={styles.imagetop}>
                      <Text style={styles.texts}>Grandview Pool</Text>
            <Text style={styles.texts2}>16855 24 Ave, Surrey, BC</Text>
        </ImageBackground>
            </View>
          <View> 
            <Text style={styles.texts3}>Accessibility</Text>
         </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-evenly'}}>
            <Image style={styles.disability} 
            source={require('../assets/hands.png')}/>
            <Image style={styles.disability} 
            source={require('../assets/noSee.png')}/>
            <Image style={styles.disability} 
            source={require('../assets/wheelchair.png')}/>
            <Image style={styles.disability} 
            source={require('../assets/autism.png')}/>
            </View>
            <View>
              <Text style={styles.texts3}>Overall Rating</Text>
              <Image
              source={require('../assets/5star.png')}
              style={{height: 50, width: 300, alignSelf: 'center', marginTop: 15 }}/>
            </View>
            <View style={{paddingTop: 30}}>
              <Text style={styles.texts3}>Reviews</Text>
              <Text style={styles.texts4}>Roy Huang</Text>
              <Image
              source={require('../assets/5star.png')}
              style={{ height: 12.5, width: 75, alignSelf: 'flex-start', marginTop: 5}} />
              <Text style={styles.texts5}>This pool is very easy to enter and is very accessible. It is also very clean and has very accomidating staff. </Text>
            </View>
            <View style={{margin: 15}}>
              <Button
              title="More Reviews"
              style={styles.buttons}/>
            </View>
            <View style={{margin: 10}}>
            <Button
            onPress={() => navigate('Review')}
            title="Write a Review"
            />
          </View>
        </ScrollView>



        )
    }}


    const styles = StyleSheet.create({

        containerTop: {
          flex: 1,
          resizeMode: 'cover',
          justifyContent:'space-around',
          alignItems: 'center',
        },
        imagetop: {
          flex: 1,
          resizeMode: 'cover',
          justifyContent: 'center',
          height: 200,
          width: "100%",
        },
        texts: {
            fontSize: 50,
            color: 'white',
            fontStyle: 'italic',
            fontWeight: 'bold',
            justifyContent: 'center',
          },
          texts2: {
            fontSize: 25,
            color: 'white',
            fontWeight: 'bold',
            justifyContent: 'center',
            paddingTop: 75
            
        },
        disability: {
          height: 50,
          width: 50,
          alignSelf: 'center',
          marginTop: 20,
          borderWidth: 1,
          borderRadius: 9
          
        },
        texts3: {
          fontSize: 20,
          color: 'black',
          fontWeight: 'bold',
          paddingTop: 20,
         alignSelf: 'center'
          
      },
      texts4: {
        fontSize: 15,
        color: 'black',
        fontWeight: 'bold',
        paddingTop: 10,
       alignSelf: 'flex-start'
        
    },
    texts5: {
      fontSize: 15,
      color: 'black',
      paddingTop: 10,
     alignSelf: 'flex-start'
      
  },
  buttons: {
    margin: 50,
  },

        })



